...
void check_ir()
{
  ...
  {
    while(digitalRead(out_ir)==1){} //wait untill person walks away from the sensor
      
    if(check==1&&in==1) //this means person was inside the room exit
    {
       if(people>0)people--;
    ...
    else //means person is out of the room and will enter it
    {
      check=1;
      out=1;
    }
   ...
   int read_in_ir=digitalRead(in_ir); //take read of in ir
    ...
    {
      while(digitalRead(in_ir)==1){} //wait untill person walks away from the sensor
      if(check==1&&out==1) //this means person enter the room
      {
         people++;
      ...
      else //means person inside the room and will exit 
      {
        check=1;
        in=1;
      }
...
void buttons()
{  
  //take the read of button to determined what action should do
